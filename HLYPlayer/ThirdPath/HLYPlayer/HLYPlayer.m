...
- (instancetype)initWithFrame:(CGRect)frame videoURLStr:(NSString *)videoURLStr{
    ...
    if (self) {
        ...
        //crutimeLabel
        self.crutimeLabel = [[UILabel alloc]init];
        self.crutimeLabel.textAlignment = NSTextAlignmentRight;
        self.crutimeLabel.textColor = [UIColor whiteColor];
        self.crutimeLabel.backgroundColor = [UIColor clearColor];
        self.crutimeLabel.font = [UIFont systemFontOfSize:11];
        [self.bottomView addSubview:self.crutimeLabel];
        //autoLayout timeLabel
        [self.crutimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
...
//        [self.progressSlider addTarget:self action:@selector(updateProgress:) forControlEvents:UIControlEventTouchUpInside];
       ...
       //_playOrPauseBtn
        ...
        [self.playOrPauseBtn setImage:[UIImage imageNamed:(@"play_press")] ?: [UIImage imageNamed:(@"play_press")] forState:UIControlStateNormal];
        [self.playOrPauseBtn setImage:[UIImage imageNamed:(@"plays")] ?: [UIImage imageNamed:(@"plays")] forState:UIControlStateSelected];
        ...
        [self.playOrPauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(self).with.offset(66);
            make.width.mas_equalTo(50);
        }];
        _loadFaildLabel =[[UILabel alloc] init];
        _loadFaildLabel.textColor=[UIColor whiteColor];
        _loadFaildLabel.textAlignment=NSTextAlignmentCenter;
        [self addSubview:_loadFaildLabel];
        
        [_loadFaildLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo((SCREEN_FRAME.size.width-250)/2);
            make.height.mas_equalTo(20);
            make.bottom.equalTo(_playOrPauseBtn).with.offset(25);
            make.width.mas_equalTo(250);
        }];
        _loadFaildLabel.hidden=YES;
...
{
    CGPoint location1 = [recognizer locationInView:self.progressSlider];
    self.progressSlider.value = [self valueForPosition:location1];
...
-(void)showActivityIndicatorView:(UIActivityIndicatorView *)activityView andFrame:(CGRect)fram inView:(UIView *)view{
    [view addSubview:activityView];
    [activityView stopAnimating];
    [activityView setCenter:CGPointMake(fram.size.width / 2, fram.size.height/2)];
...
- (void)handleSingleTap{
    [UIView animateWithDuration:0.5 animations:^{
        if (self.bottomView.alpha == 0.0) {
            ...
            [self.autoDismissTimer timeInterval];
        }
