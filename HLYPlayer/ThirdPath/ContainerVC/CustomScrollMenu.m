...
@interface CustomScrollMenu()<CustomScrollerMenuDelegate,UIScrollViewDelegate>
    //滚动视图
...
@end
@implementation CustomScrollMenu
...
-(void)loadDataAndView
{
    ...
    self.selectedColor=[UIColor colorWithHexString:@"#8c8c8c"];
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    for (NSString *title in titles)
    {
        ...
        CGFloat eachButtonWidth = size.width + 40.f;
...
- (void)setCurrentIndex:(NSInteger)currentIndex
{
    if (_items.count>0) {
        if (_currentIndex!=currentIndex) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"closeTheVideo" object:nil];
        }
        _currentIndex = currentIndex;
        
        UIButton *button = nil;
        UIButton *currButon=_items[currentIndex];
        for(int i=0;i<_items.count;i++){
            button = _items[i];
            [button setTitleColor:(i==currentIndex)?self.selectedColor:self.noSlectedColor forState:UIControlStateNormal];
        }
        
        CGFloat offsetX = currButon.center.x - self.navigationTabBarWidth * 0.5;
        ...
        [UIView animateWithDuration:.2f animations:^{
            _line.frame = CGRectMake(currButon.frame.origin.x + 2.0f, _line.frame.origin.y, [_itemsWidth[currentIndex] floatValue] - 4.0f, _line.frame.size.height);
        }];
    }
}
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(2.0f, self.bounds.size.height - 4.0f, width - 4.0f, 3.0f)];
